...
const Chatcontainer = () => {
  ...
  const { selectedUser, setSelectedUser, messages, sendMessages, getMessages, authUser, onlineUsers} = useAppContext()
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
     ...
     <div className='flex flex-1 items-center bg-gray-100/12 px-3 rounded-full'>
      <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" ? handleSendMessage(e) : null} type="text" placeholder='Send a message' className='flex-1 text-sm p-3 border-none rounded-lg outine-none text-white placeholder-gray-400 outline-0' />
